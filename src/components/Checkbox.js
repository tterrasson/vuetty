...
/**
 * Checkbox component - Interactive multi-select checkbox list
 */
export default {
  name: 'Checkbox',
  props: {
  ...
  setup(props, { emit }) {
    ...
    const componentId = `checkbox-${++checkboxIdCounter}`;
    ...
    // Initialize: Highlight first non-disabled option
    jumpToFirst();
 ...
 * Helper function
 */
function renderCheckboxVertical(props) {
  const {
    options = [],
    modelValue = [],
    label = '',
  ...
  // Default: maxWidth + 4 (indicator) + 2 (padding/spaces)
  const innerWidth = props.width ? props.width : (maxWidth + 6);

  // Calculate width available for text content
  // Layout: " " + indicator(3) + " " + text + " "
  // Total overhead = 1 + 3 + 1 + 1 = 6 chars
  ...
  for (let i = 0; i < Math.min(height, options.length); i++) {
    ...
    if (option) {
      ...
      const isSelected = modelValue.includes(option.value);
      ...
      // Layout: | + space + indicator + space + text + space + |
 ...
 * Helper function
 */
function renderCheckboxHorizontal(props) {
  const {
    options = [],
    modelValue = [],
    label = '',
...
export function renderCheckbox(props) {
  ...
  if (direction === 'horizontal') {
    return renderCheckboxHorizontal(props);
  } else {
    return renderCheckboxVertical(props);
  }
}

/**
 * Render handler for checkbox
 */
class CheckboxRenderHandler extends RenderHandler {
  render(ctx) {
    const { node } = ctx;
    ...
    const output = renderCheckbox(ctx.props);
...
renderHandlerRegistry.register('checkbox', new CheckboxRenderHandler());
